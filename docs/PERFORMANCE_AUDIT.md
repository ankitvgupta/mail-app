...
The sandbox has no test-account credentials. Live checks can run on the Mac where `.env.local` already contains the dedicated account's OAuth credentials; no token needs to enter chat or this sandbox. The test helper checks the authenticated address against `EXOEMAILTEST_EMAIL` before proceeding. The current full-sync test needs repair before relying on it: it passes the unsupported `APP_DATA_DIR_OVERRIDE` variable and resolves the built app relative to the wrong directory. Use the supported `EXO_USER_DATA_DIR` and prime that isolated directory with test-account credentials before a future live sync run.
